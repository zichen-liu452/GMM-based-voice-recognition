...
[ignore,perm]=sort(rand(1,data_sz)); %Generate random numbers in any order
...
for n=1:kiter
  ...
  old_centres=centres; %Store old centers to facilitate calculation of termination conditions
  ...
  if n > 1
    ...
    if max(max(abs(centres - old_centres))) < 0.001 & abs(old_e - e) < 0.001
      return;
    end
  end
  old_e = e;
